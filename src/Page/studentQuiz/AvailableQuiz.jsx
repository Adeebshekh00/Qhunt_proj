...
const AvailableQuiz = () => {
  ...
  const handleTakeQuiz = (quizName) => {
    setSelectedQuiz(null); // Reset selectedQuiz state
    ...
    <div className="flex flex-col mt-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {quizzes.map((quiz, index) => (
          <div 
            key={index} 
            className={`relative bg-gray-100 p-6 rounded-lg shadow-md border border-gray-300 transition-transform hover:shadow-lg hover:bg-gray-200 cursor-pointer transform hover:scale-105 ${index === selectedQuiz ? 'z-10' : 'z-0'}`}
